...
class BeamForming:
    ...
    def adjust_plot_limits(self):
        ...
        if self.state['scenario'] == "5G":
            self.map_ax.set_xlim(-2, 2)
            self.map_ax.set_ylim(0, 2)
            # Adjust ticks to a finer resolution
            self.map_ax.set_xticks(np.arange(-2, 2.1, 0.5))  # Ticks every 0.2 units
            self.map_ax.set_yticks(np.arange(0, 2.1, 0.5))  # Ticks every 0.2 units
        elif self.state['scenario'] == "Ultrasound":
            self.map_ax.set_xlim(-2, 2)
            self.map_ax.set_ylim(0, 2)
            # Adjust ticks for the Ultrasound scenario
            self.map_ax.set_xticks(np.arange(-2, 2.1, 0.5))  # Ticks every 0.5 units
            self.map_ax.set_yticks(np.arange(0, 2.1, 0.5))  # Ticks every 0.5 units
        elif self.state['scenario'] == "Tumor Ablation":
            self.map_ax.set_xlim(-3, 3)
            self.map_ax.set_ylim(0, 2)
            # Adjust ticks for Tumor Ablation
            self.map_ax.set_xticks(np.arange(-3, 3.1, 1))  # Ticks every 1 unit
            self.map_ax.set_yticks(np.arange(0, 2.1, 0.5))  # Ticks every 0.5 units
        else:
            # Default scenario
